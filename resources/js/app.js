...
$("#btn-next").click(function() {
    ...
    if ($(".diet-step-3").hasClass("active")) {
        ...
        if (nomeat == "nomeat" || meat.length != 0) {
            ...
            $("#progress-bar").css("width", "50%");
            $(".process-4").addClass("m-progress-bar__dot--active");
            $(".m-progress-bar__dot--active").css(
                "background-color",
                "#78bf4d"
            );
        }
    ...
    if ($(".diet-step-7").hasClass("active")) {
        ...
        if ($("#material_measurement").hasClass("active")) {
            if (
                parseFloat($("#age").val()) > 14 &&
                parseFloat($("#age").val()) < 99 &&
                parseFloat($("#height").val()) > 135 &&
                parseFloat($("#height").val()) < 255 &&
                parseFloat($("#weight").val()) > 40 &&
                parseFloat($("#weight").val()) < 180 &&
                parseFloat($("#target_weight").val()) > 40 &&
                parseFloat($("#target_weight").val()) < 180
            ) {
                if (
                    parseFloat($("#weight").val()) <
                    parseFloat($("#target_weight").val())
                ) {
            ...
            if (
                parseFloat($("#imperial_age").val()) > 14 &&
                parseFloat($("#imperial_age").val()) < 99 &&
                parseFloat($("#ft").val()) > 4 &&
                parseFloat($("#ft").val()) < 8 &&
                parseFloat($("#inch").val()) < 11 &&
                parseFloat($("#imperial_weight").val()) > 90 &&
                parseFloat($("#imperial_weight").val()) < 400 &&
                parseFloat($("#imperial_target_weight").val()) > 90 &&
                parseFloat($("#imperial_target_weight").val()) < 400
            ) {
                if (
                    parseFloat($("#imperial_weight").val()) <
                    parseFloat($("#imperial_target_weight").val())
                ) {
...
$(".svg-select").click(function() {
    ...
    if ($(".diet-step-4").hasClass("active")) {
        if ($(this).hasClass("checked")) {
            $(this).removeClass("checked");
            vegetable.splice(
                $.inArray($(this).attr("data-products"), vegetable),
                1
            );
        } else {
            $(this).addClass("checked");
    ...
    if ($(".diet-step-5").hasClass("active")) {
        if ($(this).hasClass("checked")) {
            $(this).removeClass("checked");
            products.splice(
                $.inArray($(this).attr("data-products"), products),
                1
            );
        } else {
            $(this).addClass("checked");
...
$("#btn_show_result").click(function() {
    ...
    if(show_result[5] <= 18) {
        ...
        $("#Oval-2").css({"color": "rgb(0, 173, 238)"});
        $("#Oval-2").css({"fill": "rgb(0, 173, 238)"});
        $("#diet_bmi").css({"color": "rgb(0, 173, 238)"});
        $("#diet_bmi").css({"fill": "rgb(0, 173, 238)"});
        $("#diet_bmio").css({"color": "rgb(0, 173, 238)"});
    }

    if(show_result[5] > 25 && show_result[5] <= 32) {
        $("#Oval-2").attr("cx", "296.807");
        $("#Oval-2").attr("cy", "339.42");
        $("#Oval-2").css({"color": "rgb(129, 194, 47)"});
        $("#Oval-2").css({"fill": "rgb(129, 194, 47)"});
        $("#diet_bmi").css({"color": "rgb(129, 194, 47)"});
        $("#diet_bmi").css({"fill": "rgb(129, 194, 47)"});
        $("#diet_bmio").css({"color": "rgb(129, 194, 47)"});
    }

    if(show_result[5] > 32 && show_result[5] <= 40) {
        $("#Oval-2").attr("cx", "433.056");
        $("#Oval-2").attr("cy", "301.192");
        $("#Oval-2").css({"color": "rgb(243, 174, 4)"});
        $("#Oval-2").css({"fill": "rgb(243, 174, 4)"});
        $("#diet_bmi").css({"color": "rgb(243, 174, 4)"});
        $("#diet_bmi").css({"fill": "rgb(243, 174, 4)"});
        $("#diet_bmio").css({"color": "rgb(243, 174, 4)"});
    }

    if(show_result[5] > 41) {
        $("#Oval-2").attr("cx", "484.014");
        $("#Oval-2").attr("cy", "274.628");
        $("#Oval-2").css({"color": "rgb(194, 19, 0)"});
        $("#Oval-2").css({"fill": "rgb(194, 19, 0)"});
        $("#diet_bmi").css({"color": "rgb(194, 19, 0)"});
        $("#diet_bmi").css({"fill": "rgb(194, 19, 0)"});
        $("#diet_bmio").css({"color": "rgb(194, 19, 0)"});
    }
  
    if(show_result[2] > 30 && show_result[2] < 45 ) {
      $("#graph-2").css({transform: 'matrix(1, 0, 0, 1, -130, 0)'});
      $("#young").removeClass("m-item-summary__graph-2-item--active");
      $("#young-svg").css({"opacity": "0.5"});
      $("#young-svg").css({transform: 'matrix(0.7, 0, 0, 0.7, 0, -30)'})
      $("#middle-age").addClass("m-item--summary__graph-2-item--active");
      $("#middle-age-svg").css({"opacity": "1"});
      $("#middle-age-svg").css({transform: 'matrix(1, 0, 0, 1, 0, 0)'});
      $(".stroke-2").css("stroke", "#f3da1a");
      $("#old-age-svg").css({"opacity": "0.5"});
      $("#old-age-svg").css({transform: 'matrix(0.7, 0, 0, 0.7, 0, -30)'});
      $("#older-age-svg").css({"opacity": "0.2"});
      $("#older-age-svg").css({transform: 'matrix(0.5, 0, 0, 0.5, 0, -50)'});
      $(".js-item-color-graph-2").css("color", "#f3da1a");
    }
  
    if(show_result[2] >= 45 && show_result[2] < 65 ) {
      $("#graph-2").css({transform: 'matrix(1, 0, 0, 1, -260, 0)'});
      $("#young").removeClass("m-item-summary__graph-2-item--active");
      $("#young-svg").css({"opacity": "0.2"});
      $("#young-svg").css({transform: 'matrix(0.5, 0, 0, 0.5, 0, -50)'})
      $("#middle-age-svg").css({"opacity": "0.5"});
      $("#middle-age-svg").css({transform: 'matrix(0.7, 0, 0, 0.7, 0, -30)'});
      $("#old-age").addClass("m-item--summary__graph-2-item--active");
      $(".stroke-3").css("stroke", "#ffbb42");
      $("#old-age-svg").css({"opacity": "1"});
      $("#old-age-svg").css({transform: 'matrix(1, 0, 0, 1, 0, 0)'});
      $("#older-age-svg").css({"opacity": "0.5"});
      $("#older-age-svg").css({transform: 'matrix(0.7, 0, 0, 0.7, 0, -30)'});
      $(".js-item-color-graph-2").css("color", "#ffbb42");
    }
  
    if(show_result[2] >= 65) {
      $("#graph-2").css({transform: 'matrix(1, 0, 0, 1, -390, 0)'});
      $("#young").removeClass("m-item-summary__graph-2-item--active");
      $("#young-svg").css({"opacity": "0.2"});
      $("#young-svg").css({transform: 'matrix(0, 0, 0, 0, 0, -50)'})
      $("#middle-age-svg").css({"opacity": "0.2"});
      $("#middle-age-svg").css({transform: 'matrix(0.5, 0, 0, 0.5, 0, -50)'});
      $(".stroke-4").css("stroke", "#f94b2c");
      $("#old-age-svg").css({"opacity": "0.5"});
      $("#old-age-svg").css({transform: 'matrix(0.7, 0, 0, 0.7, 0, -30)'});
      $("#older-age").addClass("m-item--summary__graph-2-item--active");
      $("#older-age-svg").css({"opacity": "1"});
      $("#older-age-svg").css({transform: 'matrix(1, 0, 0, 1, 0, 0)'});
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (response.success == true) {
                ...
                for (i = 0; i < 7; i++) {
                    (row1 =
                        '<li class="nav-item week-num">\
                    <a class="one-week nav-link" href="#tab' +
                        (i + 1) +
                        '" data-toggle="tab"> ' +
                        (start_date + i) +
                        "</a>\
                ...
                for (i = 0; i < 7; i++) {
                    ...
                    <div class="media-body">\
                        <h5>' +
                            this[i].name +
                            '</h5>\
                        <div class="btn-timetable diet_detail" data-recipe-id="' +
                            this[i].id +
                            '">\
